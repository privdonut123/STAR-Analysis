...
void StFttQAMaker::MakeClusterQA(){
    ...
    for ( auto rawHit : mFttCollection->rawHits() ) {
        ...
        if ( rawHit->tb() < -70 || rawHit->tb() > 200 ) continue;
        nGoodHits++;
        hitProj[index].push_back( rawHit );
    }

    LOG_INFO << "StFttQAMaker found " << nGoodHits << " num good hits vs. size() == " << mFttCollection->rawHits().size() << endm;
...
StFttQAMaker::BookTree()
{
    ...
    mFttTree->Branch("fullN"    , &mFttData.fullN   , "fullN/s");
