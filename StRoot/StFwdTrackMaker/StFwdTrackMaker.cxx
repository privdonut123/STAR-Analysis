...
int StFwdTrackMaker::Make() {
    ...
    StEvent *stEvent = static_cast<StEvent *>(GetInputDS("StEvent"));
    if (!stEvent) {
        LOG_WARN << "Cannot Get StEvent, Skip Fwd Tracking" << endm;
        return kStOk;
    }


    double vpdVz = -999;
    int nTofCut = mFwdConfig.get<int>( "EventSelection:nTof", 0 );
    bool requireVpdVz = mFwdConfig.get<bool>( "EventSelection:vpdVz", false );
    if ( requireVpdVz || nTofCut > 0 ){
        ...
        bool vpdInBounds = fabs(btofHeader->vpdVz()) < 150.0;
        vpdVz = btofHeader->vpdVz();
        ...
        if ( nTof < nTofCut ){
            LOG_INFO << "SKIP FWD nTOF" <<endm;
            return kStOk;
        }

        if ( requireVpdVz && ( nEast <= 1 || nWest <= 1 || !vpdInBounds ) ){
            LOG_INFO << "SKIP FWD TRACKING DUE TO VPD" <<endm;
            return kStOk;
        }
    ...
    if ( fabs(vpdVz) < 100 )
        mForwardTracker->setEventVertex( TVector3( 0, 0, vpdVz ) );
    else 
...
void StFwdTrackMaker::FillTrackDeltas(){
    ...
    for ( size_t i = 0; i < fittedTracks.size(); i++ ){
        auto st = fittedTracks[i].fttSeed;
