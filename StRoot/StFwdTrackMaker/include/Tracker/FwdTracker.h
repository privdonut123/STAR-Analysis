...
public:
    GenfitTrackResult(   size_t nFTT, size_t nFST,
                        Seed_t &fttSeed, Seed_t &fstSeed, genfit::Track *track ) {
        this->nFST = nFST;
        this->nFTT = nFTT;
...
class ForwardTrackMaker {
  public:
    ...
    */
    ...
    void fitTrack(Seed_t &trackSeed) {
        ...
        idt = MCTruthUtils::dominantContribution(trackSeed, qual);
    
        TVector3 mcSeedMom;
        auto mctm = mDataSource->getMcTracks();
        
        // get the MC track momentum if we can (may be used as seed per options)
        if (mctm.count(idt)) {
            auto mct = mctm[idt];
        ...
        if (true == mConfig.get<bool>("TrackFitter:mcSeed", false)) {
            ...
            p = mTrackFitter->fitTrack(trackSeed, pVertex, &mcSeedMom);
        } else {
            // Normal case, real data
            p = mTrackFitter->fitTrack(trackSeed, pVertex);
        }
        ...
        Seed_t fstSeeds; // none
        GenfitTrackResult gtr( trackSeed.size(), 0, trackSeed, fstSeeds, genTrack );

        // assign the fit results to be saved
        fitStatus = mTrackFitter->getStatus();
        trackRep = mTrackFitter->getTrackRep()->clone(); // Clone the trackSeed rep
    ...
    */
    ...
    void addFttHits() {
        ...
        for ( auto t : trs ){
            ...
            try {
                ...
                hits_near_plane3 = findFttHitsNearMe(hitmap[3], msp3);
                hits_near_plane2 = findFttHitsNearMe(hitmap[2], msp2);
                hits_near_plane1 = findFttHitsNearMe(hitmap[1], msp1);
                hits_near_plane0 = findFttHitsNearMe(hitmap[0], msp0);
            ...
            if ( hits_to_add.size() > 0 ){
                ...
                Seed_t combinedSeed;
                LOG_INFO << "Found " << t.fttSeed.size() << " existing FST seed points" << endm;
                combinedSeed.insert( combinedSeed.begin(), t.fttSeed.begin(), t.fttSeed.end() ); // this is goofed but will fix
                ...
                if ( lengthGTR >= 1 ){
                    ...
                    LOG_INFO << "Track Refit with " << mTrackFitter->getTrack()->getNumPoints() << " points" << endm;
                } else {
    ...
    Seed_t findFttHitsNearMe(Seed_t &available_hits, genfit::MeasuredStateOnPlane &msp, double dx = 1.5, double dy = 1.5) {
        // double probe_phi = TMath::ATan2(msp.getPos().Y(), msp.getPos().X());
        // double probe_r = sqrt(pow(msp.getPos().X(), 2) + pow(msp.getPos().Y(), 2));
